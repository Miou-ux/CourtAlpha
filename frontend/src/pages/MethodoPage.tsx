...
function formatRoiPct(value: number): string {
  const sign = value >= 0 ? '+' : ''
...
function mapApiRows(
  ...
  return rows.map((r) => ({
    ...
    bets: r.bets,
    hitPct: r.hit_pct,
    roiYearPct: r.roi_year_pct,
    roi1d1pPct: r.roi_1d1p_pct,
    brier: r.brier,
    note:
      r.note === 'partial'
        ? isEn
          ? 'Partial year (Jan–May)'
          : 'Année partielle (janv.–mai)'
        : r.note ?? null,
  }))
}
...
export function MethodoPage() {
  ...
  const statsQ = useQuery({
    queryKey: ['methodo-yearly-stats'],
    queryFn: () => api.picksMethodoYearlyStats(),
    staleTime: 60 * 60 * 1000,
    retry: 1,
  })

  const backtestRows: BacktestRow[] = useMemo(() => {
    if (statsQ.data?.rows?.length) {
      return mapApiRows(statsQ.data.rows, isEn)
    }
    return content.METHODO_BACKTEST_ROWS.map((row) => ({
      year: row.year,
      bets: row.bets,
      hitPct: row.hitPct,
      roiYearPct: row.roiYearPct,
      roi1d1pPct: row.roi1d1pPct,
      brier: row.brier,
      note: row.note,
    }))
  }, [statsQ.data, content.METHODO_BACKTEST_ROWS, isEn])

  const bestHit = useMemo(
    () => backtestRows.reduce((best, r) => (r.hitPct > best.hitPct ? r : best), backtestRows[0]),
    [backtestRows],
  )
  const bestRoiYear = useMemo(
    () => backtestRows.reduce((best, r) => (r.roiYearPct > best.roiYearPct ? r : best), backtestRows[0]),
    [backtestRows],
  )
  const bestRoi1d1p = useMemo(
    () => backtestRows.reduce((best, r) => (r.roi1d1pPct > best.roi1d1pPct ? r : best), backtestRows[0]),
    [backtestRows],
  )
    ...
    <div className="space-y-8">
      ...
      <section>
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-border bg-bg-elevated text-[11px] uppercase tracking-wide text-muted">
              <tr>
                ...
                <th className="px-4 py-3">{t('methodo.colRoiYear')}</th>
                <th className="px-4 py-3">{t('methodo.colRoi1d1p')}</th>
            ...
            <tbody>
              {backtestRows.map((row) => (
                <tr key={row.year} className="border-b border-border/70 last:border-0">
                  ...
                  <td className="quant px-4 py-3 text-success">{formatRoiPct(row.roiYearPct)}</td>
                  <td className="quant px-4 py-3 text-success">{formatRoiPct(row.roi1d1pPct)}</td>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <StatTile
            label={t('methodo.bestHit')}
            value={`${bestHit?.hitPct.toFixed(1).replace('.', ',')} %`}
            className="px-4 py-3"
          />
          <StatTile
            label={t('methodo.bestRoiYear')}
            value={formatRoiPct(bestRoiYear?.roiYearPct ?? 0)}
            className="px-4 py-3"
          />
          <StatTile
            label={t('methodo.bestRoi1d1p')}
            value={formatRoiPct(bestRoi1d1p?.roi1d1pPct ?? 0)}
            className="px-4 py-3"
          />
        </div>
      </section>
